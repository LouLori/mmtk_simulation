...
c       write(6,*)
        ...
        pre=dfloat(2*j+1)/(8.d0*pi*pi)  ! Pre constant for density matrix
c       write(6,*)'pre=',pre
        rho1=0.d0 ! The loop over J(0,infty)
        erot1=0.d0
        esq1=0.d0
        do ist=1,ndimev
          istevn=istevn+1
          energy=engevn(istevn)  ! Calculate the eigenenergies of the asymmetric top
          expo=exp(-tau*energy)  ! The exponential part for rho
...
c         write(6,*)j,ist,ivec,energy
          im=0
          rho2=0.d0 ! The loop over M(-J,J)
          do m=kevnst,j,2
            im=im+1
            coef1=eigevn(im+ivec)   ! The A coef1
            ik=0
            rho3=0.d0 ! The loop over K(-J,J)
c           write(6,*)'coef1=',coef1
            do k=kevnst,j,2
              ik=ik+1
              coef2=eigevn(ik+ivec) ! The A coef2
              rho3=rho3+coef2*dlist(j,m,k,ithe)*
     +                        cos(dfloat(m)*phi+dfloat(k)*chi)   ! dlist(): Wigner func; cos(): cos part in rho
            enddo
c           write(6,*)coef2,rho3
            rho2=rho2+coef1*rho3
          enddo
          rho1=rho1+rho2*expo ! The rotational density matrix
