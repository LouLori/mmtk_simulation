...
cdef class HarmonicAngleTerm(EnergyTerm):
    ...
    def __init__(self, universe, atom_indices, theta_eq, k_theta):
    ...
    cdef void evaluate(self, PyFFEvaluatorObject *eval,
        ...
        #Add energy to MMTK
        energy.energy_terms[self.index] = 0.5*self.k_theta*(pow(dtheta,2.))

        #Add gradients
        if energy.gradients != NULL:
            #Determine dV wrt theta
            sin_theta = sqrt(1. - pow(cos_theta,2.))
            deriv = -1.*self.k_theta*dtheta/sin_theta
...
cdef class HarmonicBondTerm(EnergyTerm):
    ...
    def __init__(self, universe, atom_indices, r_eq, k_r):
        cdef ArrayType ref_array
        EnergyTerm.__init__(self, universe,
    ...
    cdef void evaluate(self, PyFFEvaluatorObject *eval,
        ...
        #Add energy to MMTK
        energy.energy_terms[self.index] = 0.5*self.k_r*(pow(dr,2.))
...
cdef class QuarticBondTerm(EnergyTerm):
    ...
    def __init__(self, universe, atom_indices, D_r, c1, c2, c3, r_eq):
        cdef ArrayType ref_array
        EnergyTerm.__init__(self, universe,
    ...
    cdef void evaluate(self, PyFFEvaluatorObject *eval,
        ...
        #Add energy to MMTK
        energy.energy_terms[self.index] = self.D_r*(self.c1*pow((dr),2.0) - self.c2*pow(dr,3.0) + self.c3*pow(dr,4.0))
        
        #Add gradients
        if energy.gradients != NULL:
            deriv = -1.*self.D_r*(2.*self.c1*dr -3.*self.c2*pow(dr,2.0) + 4.*self.c3*pow(dr,3.0))
            gradients = <vector3 *>(<PyArrayObject *> energy.gradients).data
...
cdef class ElectrostaticTerm(EnergyTerm):
    ...
    cdef long [:] atom_indices

    def __init__(self, universe, atom_indices, frac, o_charge, e_energy):
        cdef ArrayType ref_array
...
#        self.atom_indices=atom_indices
    ...
    cdef void evaluate(self, PyFFEvaluatorObject *eval,
        ...
        for i in range(0,len(self.atom_indices)-1,self.atoms_per_molecule):
            ...
            for j in range(i+self.atoms_per_molecule,len(self.atom_indices),self.atoms_per_molecule):
                ...
                if energy.gradients != NULL:
                    ...
                    #H1 H3 interaction
                    deriv = self.electrostatic_energy * self.HH / (h1h3r * h1h3r2)
                    dx = deriv * h1h3[0]
                    dy = deriv * h1h3[1]
                    dz = deriv * h1h3[2]
                    ...
                    #H1 H4 interaction
                    deriv = self.electrostatic_energy * self.HH / (h1h4r * h1h4r2)
                    dx = deriv * h1h4[0]
                    dy = deriv * h1h4[1]
                    dz = deriv * h1h4[2]
                    gradients[self.atom_indices[i]][0]   -= dx
                    ...
                    #H2 H3 interaction
                    deriv = self.electrostatic_energy * self.HH / (h2h3r * h2h3r2)
                    dx = deriv * h2h3[0]
                    dy = deriv * h2h3[1]
                    dz = deriv * h2h3[2]
                    ...
                    #H2 H4 interaction
                    deriv = self.electrostatic_energy * self.HH / (h2h4r * h2h4r2)
                    dx = deriv * h2h4[0]
                    dy = deriv * h2h4[1]
                    dz = deriv * h2h4[2]
                    gradients[self.atom_indices[i+1]][0] -= dx
                    ...
                    deriv = self.electrostatic_energy * self.MH / (h1m2r * h1m2r2)
                    dx = deriv * h1m2[0]
                    dy = deriv * h1m2[1]
                    dz = deriv * h1m2[2]
                    ...
                    deriv = self.electrostatic_energy * self.MH / (h2m2r * h2m2r2)
                    dx = deriv * h2m2[0]
                    dy = deriv * h2m2[1]
                    dz = deriv * h2m2[2]
                    ...
                    deriv = self.electrostatic_energy * self.MH / (m1h3r * m1h3r2)
                    dx = deriv * m1h3[0]
                    dy = deriv * m1h3[1]
                    dz = deriv * m1h3[2]
                    ...
                    deriv = self.electrostatic_energy * self.MH / (m1h4r * m1h4r2)
                    dx = deriv * m1h4[0]
                    dy = deriv * m1h4[1]
                    dz = deriv * m1h4[2]
                    gradients[self.atom_indices[i]][0] -= dx*self.f2        #H1 x
                    ...
                    deriv = self.electrostatic_energy * self.MM / (m1m2r * m1m2r2)
                    dx = deriv * m1m2[0]
                    dy = deriv * m1m2[1]
                    dz = deriv * m1m2[2]
                    gradients[self.atom_indices[i]][0] -= dx*self.f2        #H1 x
        ...
        energy.energy_terms[self.index] = energy_sum
...
cdef class LennardJonesTerm(EnergyTerm):
    ...
    def __init__(self, universe,atom_indices, epsilon, sigma):
        cdef ArrayType ref_array
        cdef int i
        EnergyTerm.__init__(self, universe,
    ...
    cdef void evaluate(self, PyFFEvaluatorObject *eval,
        ...
        for i in range(atoms):
            for j in range(i+1,atoms):
                ...
                if energy.gradients != NULL:
                    deriv = (12*self.A*r12i - 6.*self.B*r6i) * r2i
                    gradients = <vector3 *>(<PyArrayObject *> energy.gradients).data
